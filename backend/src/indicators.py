...
def psar(high, low, step=0.02, max_step=0.2):
    ...
    ps = np.zeros(n, dtype=float); bull = True; af = step; ep = h[0]; ps[0] = l[0]
    for i in range(1, n):
        prev_ps = ps[i-1]
        if bull:
            ps[i] = prev_ps + af * (ep - prev_ps)
            if i == 1: ps[i] = min(ps[i], l[i-1])
            else: ps[i] = min(ps[i], l[i-1], l[i-2])
            if l[i] < ps[i]:
            ...
            ps[i] = prev_ps + af * (ep - prev_ps)
            if i == 1: ps[i] = max(ps[i], h[i-1])
            else: ps[i] = max(ps[i], h[i-1], h[i-2])
            if h[i] > ps[i]:
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    macd_line, macd_sig, macd_hist = macd_tv(out["Close"], 12, 26, 9)
    out["MACD"], out["MACD_sig"], out["MACD_hist"] = macd_line, macd_sig, macd_hist
